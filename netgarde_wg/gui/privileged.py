...
from netgarde_wg.gui.tunnel_session import clear_tunnel_session, tunnel_session_valid
...
def _log_shows_tunnel_up(lines: list[str] | None = None) -> bool:
    for line in lines or _log_lines():
        if "interface utun" in line and " up" in line:
            return True
        if line.startswith("netgarde-wg: interface ") and line.endswith(" up"):
            return True
    return False
...
def _tunnel_pgrep_patterns() -> list[str]:
    if getattr(sys, "frozen", False):
        return [
            "NetGarde.app/Contents/MacOS/netgarde-wg",
            "NetGarde.app/Contents/MacOS/wireguard-go",
        ]
    return ["netgarde-wg", "wireguard-go"]
...
def stop_tunnel() -> tuple[bool, str]:
    ...
    if getattr(sys, "frozen", False):
        match_block = """
for _PATTERN in NetGarde.app/Contents/MacOS/netgarde-wg NetGarde.app/Contents/MacOS/wireguard-go; do
...
for _ in $(seq 1 40); do
  pgrep -f NetGarde.app/Contents/MacOS/netgarde-wg >/dev/null 2>&1 && sleep 0.5 && continue
  pgrep -f NetGarde.app/Contents/MacOS/wireguard-go >/dev/null 2>&1 && sleep 0.5 && continue
  break
done
for _PATTERN in NetGarde.app/Contents/MacOS/netgarde-wg NetGarde.app/Contents/MacOS/wireguard-go; do
  for _PID in $(pgrep -f "$_PATTERN" 2>/dev/null); do
...
"""
        ...
        match_block = """
for _NAME in netgarde-wg wireguard-go; do
  killall -TERM "$_NAME" 2>/dev/null
done
for _ in $(seq 1 40); do
  pgrep -x netgarde-wg >/dev/null 2>&1 && sleep 0.5 && continue
