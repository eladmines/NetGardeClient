...
from netgarde_wg.wireguard.config import (
    WireGuardConfig,
...
class Client:
    def __init__(
        ...
        *,
        ...
        if not base_url:
            raise ValueError("netgarde api: empty base URL")
        if not enroll_path:
    ...
    def fetch_block_page_ca(self) -> bytes:
        ...
        try:
            ...
            raise RuntimeError(f"netgarde api: policy CA returned {e.code}: {err_body}") from e
        except URLError as e:
            raise RuntimeError(f"netgarde api: policy CA download: {e}") from e
    ...
    def report_usage(
        ...
        *,
        ...
        try:
            ...
            err_body = e.read(MAX_ENROLL_RESPONSE_BYTES).decode(errors="replace").strip()
            raise RuntimeError(
                f"netgarde api: usage {self.usage_path} returned {e.code}: {err_body}"
            ) from e
        except URLError as e:
            raise RuntimeError(f"netgarde api: usage round trip: {e}") from e
    ...
    def enroll(self, body: EnrollRequest) -> EnrollResponse:
        ...
        try:
            ...
            err_body = e.read(MAX_ENROLL_RESPONSE_BYTES).decode(errors="replace").strip()
            raise RuntimeError(
                f"netgarde api: enroll {self.enroll_path} returned {e.code}: {err_body}"
            ) from e
        except URLError as e:
            raise RuntimeError(f"netgarde api: round trip: {e}") from e

        if status < 200 or status >= 300:
            raise RuntimeError(
                f"netgarde api: enroll {self.enroll_path} returned {status}: {raw.decode(errors='replace')}"
            )
