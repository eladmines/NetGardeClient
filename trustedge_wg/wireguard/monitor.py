...
from trustedge_wg.wireguard.stats import TransferStats, delta, mib, read_transfer_stats
...
def start_traffic_monitor(
    ...
    *,
    ...
    thread = threading.Thread(target=_loop, name="trustedge-traffic", daemon=True)
    thread.start()
    if usage_enabled:
        log.info(
